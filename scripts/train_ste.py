...
def parse_args():
    """
    To run this file use "CUDA_VISIBLE_DEVICES=3 python train_soe.py -config configs/ste/uniform_baseline.json". See
    the config file in
    the path for an example of how to construct config files.
    """
    parser = argparse.ArgumentParser(description='Run STE Experiments')
    parser.add_argument('-config', '--config_path', type=str, default='configs/ste/uniform_baseline.json', required=True,
...
def main(args):
    ...
    if hyper_search:
        ...
        logger.info('Computing STE...')

        x, loss_history, triplet_error_history, time_taken, time_history = ste.ste_adam(triplets=train_triplets_dataset.trips_data_indices,
                                                 n=n, dim=embedding_dimension, epochs=epochs, batch_size=bs,
        ...
        logger.info('Computing TSNE on a sample of points...')
        subsample = np.random.permutation(n)[0:500]
        embeddings_sample_for_tsne = x[subsample, :]
        sublabel = labels[subsample]

        X_embedded = TSNE(n_components=2, perplexity=15, learning_rate=10).fit_transform(embeddings_sample_for_tsne)
...
def run_hyper_search(config):
    """
    Important Hyperparameters for STE:
    ...
    separator = '_'
    experiment_name = 'ste_hyper_search_' + \
                      'data_' + dataset_name + \
                      '_input_dim_' + str(input_dim) + \
