...
from lib.landmark_oe import landmark_oe_with_data
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Run Landmark OE Experiments')
    parser.add_argument('-config', '--config_path', type=str, default='configs/landmark_oe/uniform_baseline.json', required=True,
...
def main(args):
    ...
    if hyper_search:
        ...
        logger.info('Computing landmark OE ...')

        x, time_taken, train_error = landmark_oe_with_data(data=vec_data, dim=embedding_dimension, trip_num=triplet_num,
...
def run_hyper_search(config):
    ...
    separator = '_'
    experiment_name = 'landmark_oe_hyper_search_' + \
                      'data_' + dataset_name + \
                      '_input_dim_' + str(input_dim) + \
    ...
    for (emb_dim, triplet_multiplier) in product(dimensions_range, triplet_multiplier_range):
        ...
        triplet_num = int(triplet_multiplier * logn * n_points * emb_dim)
        ...
        for learning_rate in learning_rate_range:
            ...
            x, time_taken, train_error = landmark_oe_with_data(data=vec_data, dim=emb_dim,
                                                               trip_num=triplet_num, batch_size=batch_size,
            ...
            results = {'train_error': train_error, 'test_error': test_error,
                       'last_embedding': x}
        ...
        result_name = 'landmark_oe_hypersearch_' + \
                      'data_' + dataset_name + \
                      '_input_dim_' + str(input_dim) + \
                      '_n_pts_' + str(n_points) + \
