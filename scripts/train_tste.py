...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Run T-STE Experiments')
    parser.add_argument('-config', '--config_path', type=str, default='configs/tste/uniform_baseline.json',
                        required=True,
...
def main(args):
    ...
    if hyper_search:
        ...
        x, loss_history, triplet_error_history, time_taken, time_history = tste.t_ste_adam(triplets=train_triplets_dataset.trips_data_indices, n=n,
                                                                             emb_dim=embedding_dimension, epochs=epochs, batch_size=bs,
...
def run_hyper_search(config):
    ...
    for (emb_dim, triplet_multiplier) in product(dimensions_range, triplet_multiplier_range):
        ...
        for learning_rate in learning_rate_range:
            ...
            x, loss_history, triplet_error_history, time_taken, time_history = tste.t_ste_adam(triplets=train_triplets_dataset.trips_data_indices, n=n,
                                                                                 emb_dim=emb_dim, epochs=epochs, batch_size=bs,
