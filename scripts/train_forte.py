...
def parse_args():
    """
    To run this file use "CUDA_VISIBLE_DEVICES=3 python train_forte.py -config configs/forte/uniform_baseline.json". See
    the config file in
    the path for an example of how to construct config files.
    """
    parser = argparse.ArgumentParser(description='Run FORTE Experiments')
    parser.add_argument('-config', '--config_path', type=str, default='configs/forte/uniform_baseline.json', required=True,
...
def main(args):
    ...
    if hyper_search:
        ...
        logger.info('Computing FORTE...')

        x, loss_history, triplet_error_history, time_taken, time_history = forte.rank_d_pgd(triplets=train_triplets_dataset.trips_data_indices, n=n,
...
def run_hyper_search(config):
    """
    Important Hyperparameters for FORTE
    ...
    separator = '_'
    experiment_name = 'forte_hyper_search_' + \
                      'data_' + dataset_name + \
                      '_input_dim_' + str(input_dim) + \
    ...
    for (emb_dim, triplet_multiplier) in product(dimensions_range, triplet_multiplier_range):
        ...
        for learning_rate in learning_rate_range:
                ...
                x, loss_history, triplet_error_history, time_taken, time_history = forte.rank_d_pgd(triplets=train_triplets_dataset.trips_data_indices, n=n,
                                                     dim=emb_dim, epochs=epochs, batch_size=batch_size, learning_rate=learning_rate,
                ...
                if test_error < best_test_error:
                    best_params_test[(emb_dim, triplet_multiplier)] = {'learning_rate': learning_rate, 'error': test_error}
                    best_test_error = test_error
                if train_error < best_train_error:
                    best_params_train[(emb_dim, triplet_multiplier)] = {'learning_rate': learning_rate,'error': train_error}
                    best_train_error = train_error

        result_name = 'forte_convergence_' + \
                      'data_' + dataset_name + \
                      '_input_dim_' + str(input_dim) + \
                      '_n_pts_' + str(n_samples) + \
    ...
    for (emb_dim, triplet_multiplier) in product(dimensions_range, triplet_multiplier_range):
        ...
        logger.info('achieved ' + str(best_on_train['error']) + ' train error with learning rate: ' + str(best_on_train['learning_rate'])
                    )
        logger.info('achieved ' + str(best_on_test['error']) + ' test error with learning rate: ' + str(best_on_test['learning_rate'])
                    )
