...
def sort_distances_pt_to_landmark(distance_mat):
    ...
    :return: sorted indices and sorted distances
    """
...
def generate_triplets_from_indices(sorted_landmarks_to_subset, sorted_indices_subset_to_landmarks, landmark_indices):
    # given the indices of items to query, generates the triplets
    triplets = []
...
def first_phase(num_landmarks, subset_size, data, dataset_size, embedding_dim,
    ...
    for index, each_subset_size in enumerate(embedding_size_intervals):
        ...
        embedding_of_subset, loss_history, _, time_soe = soe_adam(triplets=trips, n=each_subset_size,
                                                                  dim=embedding_dim,
                                                                  epochs=epochs,
...
def loss_second_phase_loss(embedding_dim, anchors_in_shells, distances_to_p, distances_to_q, lr, logger):
    # the loss function for the optimization of the second phase
...
def embedding_rest_indices(my_oracle, non_embedded_indices, embedded_indices, first_phase_embedded_points, dim, lr,
    ...
    total_time = 0
    for each_index in non_embedded_indices:
        initial_anchor_index = random.sample(range(len(embedded_indices)), 1)[0]
        dt_mat = distance_matrix(embedded_points, embedded_points)

        begin_time = time.time()

        temp_fft_anchors_per_index = fft(dt_mat, embedded_points.shape[0], initial_anchor_index)
        ...
        begin_time = time.time()
        for anchor_index in fft_anchors_per_index[0:min(2 * (dim + 1), len(fft_anchors_per_index))]:

            first_p, first_q, dt_to_p, dt_to_q = my_oracle.bulk_query_pq(un_embedded_point_index=each_index,
        ...
        non_embedded_points_index.append(each_index)
        logger.info("Embedding Progress " + str(each_index / len(non_embedded_indices) * 100) + "%")
        embeddings_of_unembedded_points.append(embedding)
    embeddings_of_unembedded_points = np.vstack(embeddings_of_unembedded_points)
    return non_embedded_points_index, embeddings_of_unembedded_points, total_time
