...
def read_file(pathName, input_filters_dict, random_state=None):
    ...
    with zipfile.ZipFile(pathName, 'r') as myzip:
        if len(rd_font) == 0:
            names = myzip.namelist()
            print('\nreading all files...please wait')
            df = pd.concat(
                apply_column_filters(pd.read_csv(myzip.open(fname, 'r')), input_filters_dict) for fname in names)
        else:
            try:
                df = pd.concat(
                    apply_column_filters(pd.read_csv(myzip.open(font + ".csv", 'r')), input_filters_dict) for font in
                    rd_font)
            except:
...
def read_df(df,
    ...
    additional_features = ['aspect_ratio', 'column_sum[]', 'upper_case', 'image', 'font_one_hot', 'fontVariant_one_hot',
                           'm_label_one_hot']
    ...
    for colName in output_feature_list:
        ...
        if colName == "aspect_ratio":
            ...
            t1 = np.array(df.loc[:, 'r0c0':],
                          dtype=dtype)  # extract the images with is everything to the right of row 0 column 0
...
def load_E13B(chars_to_train=(48, 49), columns=(9, 17), nChars=None, test_size=0, random_state=0):
   ...
   Returns
    ...
    return ds.train.features[0][:nChars], ds.train.features[1][:nChars], ds.test.features[0][:nChars], ds.test.features[
                                                                                                           1][
                                                                                                       :nChars], labels
...
def file_to_df(base_file, character_size, title="", white_space=0, input_filters_dict={}):
    ...
    df = make_df(images, character_size, character_size, originalH, originalW, tops, lefts, orientation,
                 recognized_label)
...
def plot_decision_regions(X=None, y=None, classifier=None, resolution=.005, test_idx=None, title="untitled",
                          labels=("", "")):
    '''
